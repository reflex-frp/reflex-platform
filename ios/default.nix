...
nixpkgs.runCommand "${executableName}-app" (rec {
  ...
  packageScript = nixpkgs.writeText "package" ''
    ...
    ${./PackageApplication} -v "$tmpdir/${executableName}.app" -o "$IPA_DESTINATION" --sign "$signer" --embed "$EMBEDDED_PROVISIONING_PROFILE"
    ${nixpkgs.buildPackages.darwin.xcode}/Contents/Applications/Application\ Loader.app/Contents/Frameworks/ITunesSoftwareService.framework/Versions/A/Support/altool --validate-app -f "$IPA_DESTINATION" -t ios "$@"
  '';
