...
nixpkgs.runCommand "${executableName}-app" (rec {
  ...
  '';
  portableDeployScript = nixpkgs.writeText "portable-deploy" ''
    #!/usr/bin/env bash
    set -eo pipefail
    ...
    cp ${nixpkgs.nodePackages.ios-deploy}/bin/ios-deploy $dir/ios-deploy
...
EOF
  ...
  cp --no-preserve=mode "$portableDeployScript" "$out/bin/portable-deploy"
  chmod +x "$out/bin/portable-deploy"
